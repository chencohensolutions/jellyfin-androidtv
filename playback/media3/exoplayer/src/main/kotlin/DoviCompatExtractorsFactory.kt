...
private const val LOG_TAG = "DoviCompat"
...
/** Supplies the compatibility extractor to both Kotlin and Java player implementations. */
@UnstableApi
object DoviCompat {
    @JvmStatic
    fun wrap(delegate: ExtractorsFactory): ExtractorsFactory = DoviCompatExtractorsFactory(
        delegate = delegate,
...
class DoviCompatExtractorsFactory(
    ...
    override fun createExtractors(): Array<Extractor> =
        wrap(super.createExtractors()).also {
            Log.d(LOG_TAG, "Wrapping ${it.size} default extractors")
        }

    override fun createExtractors(
        uri: Uri,
        responseHeaders: Map<String, List<String>>,
    ): Array<Extractor> = wrap(super.createExtractors(uri, responseHeaders)).also {
        Log.d(LOG_TAG, "Wrapping ${it.size} extractors for $uri")
    }
...
/** Applies the same filter to fMP4 extractors that Media3 creates for HLS media segments. */
@UnstableApi
class DoviCompatHlsExtractorFactory(
    private val delegate: HlsExtractorFactory,
    private val mode: () -> DoviCompatMode,
    private val convertNal62: (ByteArray, Int, Int) -> ByteArray?,
    private val onReport: (DoviCompatReport) -> Unit,
) : HlsExtractorFactory by delegate {
    override fun createExtractor(
        ...
        responseHeaders: Map<String, List<String>>,
        sniffingExtractorInput: androidx.media3.extractor.ExtractorInput,
        playerId: PlayerId,
    ): HlsMediaChunkExtractor = DoviCompatHlsMediaChunkExtractor(
...
@UnstableApi
private class DoviCompatHlsMediaChunkExtractor(
    private val delegate: HlsMediaChunkExtractor,
    private val mode: () -> DoviCompatMode,
    private val convertNal62: (ByteArray, Int, Int) -> ByteArray?,
    private val onReport: (DoviCompatReport) -> Unit,
) : HlsMediaChunkExtractor by delegate {
    override fun init(output: ExtractorOutput) {
        delegate.init(DoviCompatExtractorOutput(output, mode, convertNal62, onReport))
    }

    override fun recreate(): HlsMediaChunkExtractor = DoviCompatHlsMediaChunkExtractor(
        delegate.recreate(),
        mode,
        convertNal62,
        onReport,
...
private class DoviCompatExtractor(
    ...
    override fun init(output: ExtractorOutput) {
        Log.d(LOG_TAG, "Initializing ${delegate.javaClass.simpleName}")
...
private class DoviCompatTrackOutput(
    ...
    override fun format(format: Format) {
        ...
        if (isP7) {
            Log.d(LOG_TAG, "Observed profile 7 format with mode ${requestedMode.wireValue}")
        }
